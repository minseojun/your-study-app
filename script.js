...
html+=`<button class="cal-day-btn${isToday?' is-today':''}${isSel?' is-selected':''}" data-date="${ds}"> <span class="cal-strip-day">${DAYS_KO[d.getDay()]}</span> <span class="cal-strip-num">${d.getDate()}</span> <span class="cal-strip-time">${mins>0?mins+'분':''}</span> ${totalCnt>0?`<span class="cal-strip-dot" style="opacity:${doneCnt===totalCnt?1:.4}"></span>`:'<span class="cal-strip-dot" style="opacity:0"></span>'} </button>`;
});
html+=’</div></div>’;
wrap.innerHTML=html;
...
div.innerHTML=`<span class="cat-dot cat-${h.cat}"></span> <div class="habit-info"><span class="habit-text">${h.text}</span><span class="habit-days">${daysLabel}요일</span></div> <button class="habit-del-btn" data-idx="${i}">${S_DEL}</button>`;
...
bars.innerHTML+=`<div class="goal-bar-row"> <span class="goal-bar-label"><span class="cat-dot cat-${sub}"></span>${sub}</span> <div class="goal-bar-track"><div class="goal-bar-fill" style="width:${pct}%;background:${isComplete?'var(--ok)':SUBJECT_COLORS[sub]}"></div></div> <span class="goal-bar-stat${isComplete?' goal-complete':''}">${actual}<span class="goal-stat-sep">/</span>${goal}분</span> </div>`;
});
}
...
1. 수면분석: 평균수면 기반으로 학습 컨디션을 평가하고 개선 방향을 제시하라. 수면 데이터 없으면 수면 기록을 권장하라.
1. 오늘의 평가: 오늘 공부량과 집중도를 종합 평가하고 내일 개선점을 제시하라.

[출력 규칙] 이모지 금지. 한글과 숫자만 사용. 각 body는 3문장 이내. mission은 내일 실천할 구체적 행동 1문장.

{“score”:정수(0-100),“sections”:[{“icon”:””,“title”:“취약 과목 분석”,“body”:“3문장이내”},{“icon”:””,“title”:“수면 및 컨디션”,“body”:“3문장이내”},{“icon”:””,“title”:“오늘의 평가”,“body”:“3문장이내”}],“mission”:“구체적행동1문장”}`; } function renderCoachInline(parsed){ document.getElementById('inlineScoreNum').textContent=`${parsed.score}점`; document.getElementById('liveScore').textContent=parsed.score; const cs=document.getElementById('inlineCoachSections'); cs.innerHTML=''; parsed.sections.forEach(sec=>{ cs.innerHTML+=`<div class="coach-card"><div class="coach-card-head"><span>${sec.icon}</span><span>${sec.title}</span></div><div class="coach-card-body">${sec.body}</div></div>`; }); document.getElementById('inlineMissionText').textContent=parsed.mission; document.getElementById('inlineCoachState').style.display='none'; document.getElementById('inlineCoachResult').style.display='block'; } async function runCoachAnalysis(){ document.getElementById('coachRunBtn').style.display='none'; document.getElementById('inlineCoachState').style.display='flex'; try{ const raw=await callCoach(buildPrompt(collectData())); const cleaned=raw.replace(/```json\s*/gi,'').replace(/```\s*/g,'').trim(); let parsed; try{parsed=JSON.parse(cleaned);}catch{throw new Error('응답이 잘렸습니다. 잠시 후 다시 시도해주세요.');} renderCoachInline(parsed); }catch(err){ document.getElementById('inlineCoachState').style.display='none'; document.getElementById('inlineCoachError').style.display='flex'; document.getElementById('inlineCoachErrorMsg').textContent=`오류: ${err.message}`; document.getElementById(‘coachRunBtn’).style.display=‘block’;
}
}
...
{label:‘점심’,  time:‘13:00’, isBreak:true},
{label:‘5교시’, time:‘14:00’},
{label:‘6교시’, time:‘15:00’},
{label:‘7교시’, time:‘16:00’},
];

function loadTimetable(){ return lsGet(TT_KEY)||{}; }
function saveTimetable(tt){ lsSet(TT_KEY,tt); }

function renderTimetable(){
const wrap=document.getElementById(‘timetableWrap’); if(!wrap)return;
const tt=loadTimetable();
const todayDow=new Date().getDay();
const todayIdx=(todayDow>=1&&todayDow<=5)?todayDow-1:-1;

let html=’<div class="tt-table"><div class="tt-header-row"><div class="tt-period-col"></div>’;
TT_DAYS.forEach((d,i)=>{
html+=`<div class="tt-day-col${i===todayIdx?' tt-today':''}">${d}</div>`;
});
html+=’</div>’;

TT_PERIODS.forEach((p,pi)=>{
if(p.isBreak){
html+=`<div class="tt-break-row"><span>${p.label}</span><span class="tt-break-time">${p.time}</span></div>`;
return;
}
html+=`<div class="tt-row"><div class="tt-period-label"><span class="tt-period-name">${p.label}</span><span class="tt-time">${p.time}</span></div>`;
TT_DAYS.forEach((_,di)=>{
const key=`${pi}_${di}`;
const val=(tt[key]||’’).replace(/”/g,’"’);
html+=`<div class="tt-cell${di===todayIdx?' tt-today-col':''}"><input type="text" class="tt-input" value="${val}" placeholder="—" maxlength="10" data-key="${key}"/></div>`;
});
html+=’</div>’;
});
html+=’</div>’;
...
wrap.querySelectorAll(’.tt-input’).forEach(inp=>{
inp.addEventListener(‘change’,()=>{
const tt2=loadTimetable(); tt2[inp.dataset.key]=inp.value.trim(); saveTimetable(tt2);
});
inp.addEventListener(‘focus’,()=>{ inp.placeholder=’’; });
inp.addEventListener(‘blur’, ()=>{ inp.placeholder=’—’; });
});
}
